...
- ambiguous, ignored, disabled, detached, duplicate, wrong-frame, replaced, obscured, or geometrically unsafe onboarding/plugin targets; a known action whose nearest dialog ancestor is not its exact known dialog; a late or reappearing dialog after an earlier quiet sample; an unknown blocking dialog; or any pointer press after `Page.windowOpen`;
...
A fresh rc.6 profile may render public first-run dialogs over the plugin slots after page lifecycle. Handle only these exact accessible dialog/action pairs, in order when present: `内测声明` / `继续`, then `添加一个 API Key 开始使用` / `稍后配置`. Use `Accessibility.getFullAXTree` for the recorded main frame. Require exactly one non-ignored matching dialog and exactly one non-ignored, non-disabled action with positive backend identity; the action's nearest `dialog`/`alertdialog` ancestor must be that dialog. Any additional, duplicate, unknown, ignored, detached, cyclic, or ambiguous dialog/action state fails closed. Do not write local storage, profile state, configuration, or private Harness APIs to bypass onboarding. The first dialog is optional after its acknowledgment persists in the isolated Harness profile; the API dialog may recur for each fresh browser profile.

Use one absolute-deadline readiness state machine for both onboarding actions and plugin markers. Preserve the `Page.navigate` main `frameId`; resolve plugin markers with `DOM.querySelectorAll` and require exactly one. Scroll the candidate into view, choose only a finite non-degenerate in-viewport integer CSS-pixel point, and call `DOM.getNodeForLocation` with `includeUserAgentShadowDOM: false` and `ignorePointerEventsNone: false`. The hit must be in the main frame. It may be a text/icon descendant, but `Accessibility.getAXNodeAndAncestors` must prove that its chain contains the locked target backend identity and, for onboarding, the locked dialog identity. Changes to the modal set, target identity, geometry, frame, or topmost hit reset the bounded quiet window. After quiet succeeds, dispatch `mouseMoved`, then repeat a fresh AX and hit-test check before sending exactly one `mousePressed`/`mouseReleased` pair. Once pressed, never retry that action. A known dialog click must be followed by proof that the exact dialog disappeared; reappearance fails closed. Arm `Page.windowOpen` before any input and stop all further input/observation on an event. Any ambiguous target, unexpected blocking dialog, unstable readiness, wrong frame, unsafe point, topmost mismatch, or timeout fails closed without treating the plugin as observed.
