...
The current declaration plan uses one unchanged contract for RED and GREEN: first it fails because the exact packages are absent, then the same bytes pass after the frozen dependencies are installed. Both phases invoke the same final compiler arguments, including `--types node`. The frozen surface configs retain `types: []` and forbid `paths` and `typeRoots`; the command-line Node ambient input is the separately reviewed root-lock set TypeScript `6.0.3`, `@types/node` `24.13.3`, and `undici-types` `7.18.2`. It is a compiler-toolchain input, not a Harness closure member or a local declaration shim. Dependency absence alone is setup evidence; it is not RPC, slot, lifecycle, or compatibility evidence.
...
A full arbitrary-path realpath/install/replay test is conditional. It runs only when the local selected cache and disposable dependency root exist and match the committed canonical cache index/content aggregate. Task 3 must separately materialize only the exact locked Node ambient packages (`@types/node` `24.13.3` and `undici-types` `7.18.2`) for the reviewed TypeScript `6.0.3` compiler; it must not copy root `node_modules` wholesale or count those packages as Harness closure. If either replay input is absent, the test emits an explicit skip reason and the ledger records `INCONCLUSIVE_LOCAL_REPLAY_NOT_RUN`; the suite may remain green for static metadata checks, but A′-P1a may not report local replay PASS. An undeclared machine cache or the full 183 MB historical cache may never fill the gap.
