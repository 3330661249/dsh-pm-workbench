...
export function PrdPane({ state, onSelect, onCopy, onDownload }: {
  state: WorkbenchState; onSelect?: (id: PrdRevisionId) => void; onCopy?: () => void; onDownload?: () => void
}) {
  ...
  return <>
    ...
    <div className="pmwb-actions">{summaries.map((item, index) => <button type="button" key={item.prdRevisionId} data-dsh-pm-workbench="prd-history-item"
      ...
      onClick={() => onSelect?.(item.prdRevisionId)}>PRD {index + 1} · {item.status === 'current' ? '当前内容' : '历史内容'}</button>)}</div>
    ...
    <div className="pmwb-actions">
      <button type="button" data-dsh-pm-workbench="copy-prd" disabled={!selected} onClick={onCopy}>复制 Markdown</button>
      <button type="button" data-dsh-pm-workbench="download-prd" disabled={!selected} onClick={onDownload}>下载 Markdown</button>
    </div>
  </>
}
