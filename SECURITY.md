...
DSH PM Workbench is a private, unofficial experiment. Gate A is currently
**NO-GO** because the required generated Typert Host/Client Remote artifacts
were not produced by the isolated `0.1.0-rc.6` probe.

The codebase is not established as installable, Harness-compatible,
production-ready, or suitable for real interview data. Do not run it against a
production profile, expose it to the public internet, or connect it to a real
model or provider account without a later approved gate.
